...
The latest commit on `main` is the only supported version. Older commits may receive security patches at the maintainer's discretion.

## Reporting a Vulnerability

Report suspected vulnerabilities privately by emailing the maintainer at `tomasz.jakub.gorka@gmail.com` with subject prefix `[bmad-pr SECURITY]`.
...
4. Day 30+ — coordinated disclosure (fix released, then public advisory after a reasonable upgrade window).
